...
#define XLGA_CORE
#define XLGA_API extern

#define pkg_malloc malloc
#define pkg_realloc realloc
#define pkg_free free

#define xlgalua_log(level, s, ...) errx(1, s, __VA_ARGS__)

#define XLGAMYSQL_FETCH_NUM (1 << 0)
#define XLGAMYSQL_FETCH_ASSOC (1 << 1)
#define XLGAMYSQL_FETCH_BOTH (XLGAMYSQL_FETCH_NUM | XLGAMYSQL_FETCH_ASSOC)
...
static int l_xlgamysql_connect(lua_State *L)
{
  struct xlgamysql *o;
  void *ptr;
  ...
  memset(o, '\0', sizeof(*o));
  luaL_getmetatable(L, "xlgalua.mysql");
...
static void xlgamysql_free_result(struct xlgamysql *o)
{
...
static void xlgamysql_close(lua_State *L)
{
  struct xlgamysql *o;

  o = luaL_checkudata(L, 1, "xlgalua.mysql");
  if (!o->finalized && o->my)
    {
      if (o->ref != LUA_NOREF)
	{
...
static int l_xlgamysql_close(lua_State *L)
{
  xlgamysql_close(L);
  return 0;
}

static int l_xlgamysql_query(lua_State *L)
{
  struct xlgamysql *o;
  const char *str;
  int ret;
...
/* 	      xlgalua_log(L_DBG, "mysql query return a table of %d fields and %d rows", */
...
static int l_xlgamysql_affected_rows(lua_State *L)
{
  struct xlgamysql *o;
...
static int xlgamysql_fetch(lua_State *L, int result_type)
{
  struct xlgamysql *o;
  int n;
  ...
  if (o->finalized || !o->result)
    ...
    {
      if (result_type & XLGAMYSQL_FETCH_ASSOC && !o->fields)
      ...
      else
	{
	  ...
	  for (i = 0; i < o->num_fields; ++i)
	    {
	      if (result_type & XLGAMYSQL_FETCH_NUM)
		{
		  lua_pushinteger(L, i + 1);
		  lua_pushlstring(L, o->row[i], lengths[i]);
		  lua_rawset(L, -3);
		}
	      if (result_type & XLGAMYSQL_FETCH_ASSOC)
		{
...
int l_xlgamysql_fetch_row(lua_State *L)
{
  return xlgamysql_fetch(L, XLGAMYSQL_FETCH_NUM);
}

int l_xlgamysql_fetch_assoc(lua_State *L)
{
  return xlgamysql_fetch(L, XLGAMYSQL_FETCH_ASSOC);
}

int l_xlgamysql_fetch_array(lua_State *L)
{
  return xlgamysql_fetch(L, XLGAMYSQL_FETCH_BOTH);
}

int l_xlgamysql_free_result(lua_State *L)
{
  struct xlgamysql *o;
...
static int l_xlgamysql_escape(lua_State *L)
{
  struct xlgamysql *o;
  const char *str;
  size_t len;
  ...
  if (!to)
    {
      xlgalua_log(L_CRIT, "malloc of %d bytes failed", 2 * len + 1);
      lua_pushnil(L);
      return 1;
    }
...
static int l_xlgamysql_insert_id(lua_State *L)
{
  struct xlgamysql *o;
  long long id;
...
static int l_xlgamysql_prepare(lua_State *L)
{
  struct xlgamysql *o;
  struct xlgamysql_stmt *o_stmt;
  const char *str;
  size_t len;
  int ret;
  int i;
/*   my_bool arg; */
  ...
  if (o->finalized)
  ...
  else
    {
      ...
      luaL_getmetatable(L, "xlgalua.mysql_stmt");
      lua_setmetatable(L, -2);
      lua_rawgeti(L, LUA_REGISTRYINDEX, o->ref);
      lua_pushvalue(L, -2);
      lua_pushboolean(L, 1);
	  ...
	  if (o_stmt->param_count)
	    {
	      ...
	      if (!o_stmt->bind)
		{
		  xlgalua_log(L_CRIT, "malloc of %d bytes failed",
 ...
 */
static void xlgamysql_stmt_free_result(struct xlgamysql_stmt *o)
{
  int ret;
  ...
  if (!o->finalized && o->result_set_exist)
    {
      ...
      if (ret)
	xlgalua_log(L_CRIT, "mysql_stmt_free_result failed: [%d] %s",
		   mysql_stmt_errno(o->stmt), mysql_stmt_error(o->stmt));
      o->result_set_exist = 0;
    }
}

static void xlgamysql_stmt_close(lua_State *L)
{
  struct xlgamysql_stmt *o;
  int i;

  o = luaL_checkudata(L, 1, "xlgalua.mysql_stmt");
  if (!o->finalized && o->stmt)
    {
      xlgamysql_stmt_free_result(o);
...
static int l_xlgamysql_stmt_close(lua_State *L)
{
  xlgamysql_stmt_close(L);
  return 0;
}
 ...
 */
static int xlgamysql_stmt_bind(struct xlgamysql_stmt *o, lua_State *L, int n, int index)
{
  ...
  switch (lua_type(L, index))
	...
	*o->bind[n].is_null = 0;
	...
	if (!o->bind[n].buffer)
	  {
	    xlgalua_log(L_CRIT, "malloc of %d bytes failed", sizeof(number));
	    lua_pushnil(L);
	    return 1;
	  }
	...
	*o->bind[n].is_null = 0;
	...
	if (!o->bind[n].buffer)
	  {
	    xlgalua_log(L_CRIT, "malloc of %d bytes failed", len);
	    lua_pushnil(L);
	    return 1;
	  }
	...
	*o->bind[n].length = len;
    ...
    default:
      /* light & full userdata */
      xlgalua_log(L_CRIT, "invalid bind parameter #%d, Lua type %s not yet handled",
...
static int l_xlgamysql_stmt_bind(lua_State *L)
{
  struct xlgamysql_stmt *o;
  int n;
  ...
  return xlgamysql_stmt_bind(o, L, n - 1, 3);
}

static int l_xlgamysql_stmt_bind_all(lua_State *L)
{
  struct xlgamysql_stmt *o;
  int n;
  int i;
  ...
  for (i = 0; i < o->param_count; ++i)
    {
      ret = xlgamysql_stmt_bind(o, L, i, i + 2);
...
static int l_xlgamysql_stmt_execute(lua_State *L)
{
  struct xlgamysql_stmt *o;
  int ret;
	  ...
	  if (o->meta_result)
	    {
	      ...
	      if (ret) /* failure */
		{
		  xlgalua_log(L_CRIT, "mysql_stmt_bind_result failed: [%d] %s",
			     mysql_stmt_errno(o->stmt), mysql_stmt_error(o->stmt));
		  xlgamysql_stmt_free_result(o);
		  lua_pushnil(L);
		}
		  ...
		  if (ret) /* failure */
		    {
			 ...
			 a result set of 0 rows. We thus do not need to check mysql_field_count */
		      xlgalua_log(L_CRIT, "mysql_stmt_store_result failed: [%d] %s ",
				 mysql_stmt_errno(o->stmt), mysql_stmt_error(o->stmt));
		      xlgamysql_stmt_free_result(o);
		      lua_pushnil(L);
		    }
...
static int l_xlgamysql_stmt_affected_rows(lua_State *L)
{
  struct xlgamysql_stmt *o;
...
static int xlgamysql_stmt_fetch(lua_State *L, int result_type)
{
  struct xlgamysql_stmt *o;
  int n;
  int ret;
  ...
  if (o->finalized || !o->meta_result || !o->result_set_exist)
	...
	{
	  xlgalua_log(L_CRIT, "mysql_stmt_fetch failed: [%d] %s",
		     mysql_stmt_errno(o->stmt), mysql_stmt_error(o->stmt));
	  lua_remove(L, -1);
	  lua_pushnil(L);
	  return 1;
      ...
      else if (ret == MYSQL_DATA_TRUNCATED)
	{
	  ...
	  for (i = 0; i < o->num_fields; ++i)
		...
		continue;
	      ...
	      if (!buf)
		{
		  xlgalua_log(L_CRIT, "realloc of %d bytes failed", o->real_length[i]);
		  lua_remove(L, -1);
		  lua_pushnil(L);
		  return 1;
		}
	      ...
	      if (ret)
		{
		  xlgalua_log(L_CRIT, "mysql_stmt_fetch_column failed: [%d] %s",
			     mysql_stmt_errno(o->stmt), mysql_stmt_error(o->stmt));
		  lua_remove(L, -1);
		  lua_pushnil(L);
		  return 1;
...
static int l_xlgamysql_stmt_fetch_row(lua_State *L)
{
  return xlgamysql_stmt_fetch(L, XLGAMYSQL_FETCH_NUM);
}

static int l_xlgamysql_stmt_fetch_assoc(lua_State *L)
{
  return xlgamysql_stmt_fetch(L, XLGAMYSQL_FETCH_ASSOC);
}

static int l_xlgamysql_stmt_fetch_array(lua_State *L)
{
  return xlgamysql_stmt_fetch(L, XLGAMYSQL_FETCH_BOTH);
}

static int l_xlgamysql_stmt_free_result(lua_State *L)
{
  struct xlgamysql_stmt *o;
...
int l_xlgamysql___gc(lua_State *L)
{
  xlgamysql_close(L);
  return 0;
}

int l_xlgamysql_stmt___gc(lua_State *L)
{
  xlgamysql_stmt_close(L);
  return 0;
}

int l_xlgamysql___index(lua_State *L)
{
  struct xlgamysql *o;
  const char *str;
...
int l_xlgamysql_stmt___index(lua_State *L)
{
  struct xlgamysql *o;
  const char *str;
...
static const struct luaL_Reg xlgalua_mysql_mylib [] =
  {
    {"connect", l_xlgamysql_connect},
    {"close", l_xlgamysql_close},
    {"query", l_xlgamysql_query},
    {"fetch_row", l_xlgamysql_fetch_row},
    {"fetch_assoc", l_xlgamysql_fetch_assoc},
    {"fetch_array", l_xlgamysql_fetch_array},
    {"free_result", l_xlgamysql_free_result},
    {"escape", l_xlgamysql_escape},
    {"insert_id", l_xlgamysql_insert_id},
    {"affected_rows", l_xlgamysql_affected_rows},
    {"prepare", l_xlgamysql_prepare},
    {"__gc", l_xlgamysql___gc},
    {"__index", l_xlgamysql___index},
    {NULL, NULL} /* sentinel */
  };

static const struct luaL_Reg xlgalua_mysql_stmt_mylib [] =
  {
    {"close", l_xlgamysql_stmt_close},
    {"bind", l_xlgamysql_stmt_bind},
    {"bind_all", l_xlgamysql_stmt_bind_all},
    {"execute", l_xlgamysql_stmt_execute},
    {"fetch_row", l_xlgamysql_stmt_fetch_row},
    {"fetch_assoc", l_xlgamysql_stmt_fetch_assoc},
    {"fetch_array", l_xlgamysql_stmt_fetch_array},
    {"free_result", l_xlgamysql_stmt_free_result},
    {"affected_rows", l_xlgamysql_stmt_affected_rows},
    {"__gc", l_xlgamysql_stmt___gc},
    {"__index", l_xlgamysql_stmt___index},
    {NULL, NULL} /* sentinel */
  };

XLGA_API int luaopen_mysql(lua_State *L)
{
  luaL_newmetatable(L, "xlgalua.mysql");
  luaL_openlib(L, NULL, xlgalua_mysql_mylib, 0);
/*   lua_remove(L, -1); */
  luaL_newmetatable(L, "xlgalua.mysql_stmt");
  luaL_openlib(L, NULL, xlgalua_mysql_stmt_mylib, 0);
  lua_remove(L, -1);
  /* lua_pushcclosure(L, l_xlgamysql_connect, 0); */
